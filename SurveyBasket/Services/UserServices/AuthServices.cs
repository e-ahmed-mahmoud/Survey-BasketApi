...
public class AuthServices(UserManager<ApplicationUser> userManager , IJwtProvider jwtProvider) : IAuthServices
{
    ...
    public async Task<UserAuthResponse?> GetAuthTokenAsync(string email, string password, CancellationToken cancellationToken = default)
    {
        ...
        return new UserAuthResponse(user?.Id!, user?.FirstName!, user?.LastName!,
    ...
    public async Task<UserAuthResponse?> GetRefreshTokenAsync(string token, string refreshToken, CancellationToken cancellationToken = default)
    {
